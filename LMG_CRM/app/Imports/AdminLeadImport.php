...
class AdminLeadImport implements  
    ToCollection, 
    SkipsOnError, 
    WithHeadingRow, 
    WithValidation,
    SkipsOnFailure
{
    ...
    protected $assigneEmpId;

    public function __construct($empid)
    {
        $this->assigneEmpId = $empid;
    }
    ...
    public function collection(Collection $rows)
    {
        // if (!$rows->isEmpty()){
            foreach ($rows as $row) 
            {
                ...
                $uploadLeadsData["added_for"] = $this->assigneEmpId;
    ...
    public function rules() : array {
        return [
            // "*.email" => ["required", 'email', 'unique:tbl_upload_leads,email'],
            "*.contact_number" => ["required", 'unique:tbl_upload_leads,contact_number']
        ];
    }
